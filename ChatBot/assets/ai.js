...
class ConversationManager {
    ...
    getMessagesForAPI() {
        // FIX: Ensure proper message history is sent to backend
        // Filter current session messages and format for API
        const sessionMessages = this.conversations
            .filter(msg => msg.session === this.currentSession)
            .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
        
        // Return formatted messages with role and content
...
class VoiceTherapyApp {
    ...
    async sendAudioToBackend(audioBlob) {
        try {
            ...
            console.log('Audio upload response:', uploadData);
            
            if (!uploadData.audio_filepath) {
                throw new Error('Audio upload failed - no filepath returned');
            }
            
            // FIX: Get conversation history to maintain context
            const messagesHistory = this.conversationManager.getMessagesForAPI();
            console.log('Sending conversation history:', messagesHistory);
            ...
            // FIX: Validate response before handling
            if (!responseData || (!responseData.content && !responseData.audio_filepath)) {
    ...
    async handleBackendResponse(responseData) {
        // FIX: Handle transcribed text properly
        ...
        // FIX: Only use fallback if absolutely nothing is returned
        else {
            console.warn('No valid response content received');
            const fallbackMsg = "I'm having trouble processing that. Could you try again?";
            this.conversationManager.addMessage(fallbackMsg, 'assistant', 'text');
            this.addMessageToUI(fallbackMsg, 'assistant', 'text');
        }
    ...
    updateConnectionStatus(isConnected) {
        ...
        if (isConnected) {
            ...
            this.connectionStatus.innerHTML = '<div class="status-indicator"></div><span class="status-label">Connected</span>';
        } else {
            this.connectionStatus.classList.remove('connected');
            this.connectionStatus.classList.add('disconnected');
            this.connectionStatus.innerHTML = '<div class="status-indicator"></div><span class="status-label">Disconnected</span>';
        }
    }
...
class ChatTherapyApp {
    ...
    async sendMessage() {
        ...
        // FIX: Add message to conversation manager first
        ...
        try {
            // FIX: Get full conversation history for context
            const messagesHistory = this.conversationManager.getMessagesForAPI();
            console.log('Sending message with history:', messagesHistory);
            ...
            const errorMsg = 'I apologize for the technical difficulty. Please check your connection and try again.';
            this.conversationManager.addMessage(errorMsg, 'assistant', 'text');
            this.addMessageToUI(errorMsg, 'assistant', 'text');
        } finally {
            this.isLoading = false;
    ...
    async handleBackendResponse(responseData) {
        ...
        // FIX: Only use fallback when no valid response exists
        else if (responseData.content) {
            this.conversationManager.addMessage(responseData.content, 'assistant', 'text');
            this.addMessageToUI(responseData.content, 'assistant', 'text');
        } else {
            console.warn('No valid content in response');
            const fallbackResponse = "I'm having trouble processing your message. Could you try rephrasing?";
