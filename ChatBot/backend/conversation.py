...
class GeminiChatSession:
    ...
    def generate_solution(self) -> str:
        # FIX: Pass full chat history for context
        response = get_gemini_chat_completion(self.chat_history)
        # FIX: Add AI response to history
    ...
    def run_chat(self, user_messages: List[str]) -> dict:
        ...
        # FIX: Process all user messages in sequence
        for user_message in user_messages:
            warning = self.add_user_message(user_message)
            if warning:
                safety_warnings.append(warning)
                # If safety warning triggered, return immediately
                return {
